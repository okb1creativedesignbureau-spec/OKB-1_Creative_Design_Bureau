# NACA 2R1 Derived Airfoil — Single Section Pack (Root / Mid / Tip Options) | Me 109 Wing Geometry Dataset
Choose one precision-reconstructed NACA 2R1-family airfoil section from the historical Messerschmitt Bf 109 wing. Includes coordinates, plots, Excel workbook, CSV data, reference material, and a clean STL 3D model of the aerofoil.

## Overview
This package provides one (1) professionally reconstructed airfoil section from the Messerschmitt Bf 109 wing, derived from the NACA 2R1 modified profile.
Every dataset is reverse-engineered using digitized references, chamber/thickness analysis, and clean re-plotted coordinate extraction.

You may choose any 1 of these sections:

- 14.2% Root
- 12.8% Mid (Bf 109 E / Emil)
- 12.9% Mid (Bf 109 G / Gustav)
- 11.35% Tip (Bf 109 G / Gustav)
- 11.0% Tip (Bf 109 E / Emil)

This pack is meant for students, hobbyists, 3D modelers, and researchers who need a single airfoil section for modeling, printing, CNC, CFD, or comparison studies.

# Product Links

Wing root Aerofoil: 
[Bf-109's NACA 2R1 Root Aerofoil Profile Dataset for Single Aerofoil](creativedesigns840.gumroad.com/l/foypea)
  
Mid-Section Aerofoil:
[Bf-109E's NACA 2R1 mid-section Aerofoil Profile Dataset for Single Aerofoil ](https://creativedesigns840.gumroad.com/l/gknfut)

[Bf-109G's NACA 2R1 mid-section Aerofoil Profile Dataset for Single Aerofoil](https://creativedesigns840.gumroad.com/l/ksznmj)

Winngtip Section Aerofoil:
[Bf-109E's NACA 2R1 Wingtip Aerofoil Profile Dataset for Single Aerofoil](https://creativedesigns840.gumroad.com/l/owcytd)

[Bf-109G's NACA 2R1 Wingtip Aerofoil Profile Dataset for Single Aerofoil](https://creativedesigns840.gumroad.com/l/wbocdt)

## What’s Included
Each Pack A purchase includes:

1) Airfoil Geometry Data
- Full coordinate list (X/Y normalized)
- Separate CSV files:
- upper_surface.csv
- lower_surface.csv

2) Excel Workbook (Main File)
- Full plot of the profile
- Chamber/thickness visualization
- Manual reconstruction notes
- Curve smoothing & scaling functions

3) Images & Reference
- Clean reference images
- Construction diagrams
- Citation + documentation notes
- External document links (I host the references since originals are IP-protected)

4) CAD Model
- High-quality STL of the airfoil (1:1 chord)
- Suitable for printing, slicing, or importing into any CAD/CFD tool
  
## Why This Pack?

- Simple, light, and affordable
- Perfect if you only need one section
- STL format opens everywhere — zero compatibility issues
- Good for educational and experimental use

## File Formats
- .xlsx
- .csv in .txt
- .png graph diagrams
- .txt coordinates
- .stl (clean, watertight airfoil solid)

## Licensing
Personal, academic, and small commercial use allowed.

[Back To Main Page](https://okb1creativedesignbureau-spec.github.io/OKB-1_Creative_Design_Bureau/products/Bf-109-NACA-2R1.html)
[Back To Home Page](https://okb1creativedesignbureau-spec.github.io/OKB-1_Creative_Design_Bureau/)
